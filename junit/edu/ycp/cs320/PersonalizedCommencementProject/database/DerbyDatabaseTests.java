	...
	*/
	...
	public void testFindAuthorAndBookByTitle() {
		...
		//       inspected for correct content - well-formed objects with correct content
			...
			authors = new ArrayList<ZUNUSED_Author>();
	...
	public void testFindAuthorAndBookByAuthorLastName() {
		...
		//       inspected for correct content - well-formed objects with correct content
			...
			books = new ArrayList<ZUNUSED_Book>();
			for (ZUNUSED_Pair<ZUNUSED_Author, ZUNUSED_Book> authorBook : authorBookList) {
				ZUNUSED_Author author = authorBook.getLeft();
				ZUNUSED_Book book = authorBook.getRight();
				books.add(book);
	...
	public void testFindAllBooksWithAuthors() {
		...
		//       inspected for correct content - well-formed objects with correct content
			...
			books = new ArrayList<ZUNUSED_Book>();
			for (ZUNUSED_Pair<ZUNUSED_Author, ZUNUSED_Book> authorBook : bookAuthorList) {
				ZUNUSED_Author author = authorBook.getLeft();
				ZUNUSED_Book book = authorBook.getRight();
				books.add(book);
	...
	public void testFindAllAuthors() {
		...
		/*
		// get the list of (Author, Book) pairs from DB
		List<ZUNUSED_Author> authorList = db.findAllAuthors();
	...
	public void testInsertBookIntoBooksTable() {
		...
		if (book_id > 0)
		{
			...
			// to it's original state, except for using an author_id and a book_id
				...
				List<ZUNUSED_Author> authors = db.removeBookByTitle(title);				
			}
		}
		else
		{
